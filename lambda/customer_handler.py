...
def link_call_to_customer_from_analysis(call_id: str, extracted: dict | None, result: dict | None = None) -> bool:
    """
    통화 요약 결과의 extracted_info.phone/customer_name을 고객 키(phone)에 연결.
    - calls.caller_number 비어있으면 보강
    - customer_profiles upsert
    - customer_analysis 즉시 재생성
    """
    ensure_schema()
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            _ensure_column(cur, "calls", "caller_name", "caller_name VARCHAR(255) NULL")
            _ensure_column(cur, "calls", "caller_number", "caller_number VARCHAR(30) NULL")

            cur.execute("""
                SELECT user_id, store_id, caller_number, caller_name
                FROM calls
                WHERE id = %s
                LIMIT 1
            """, (call_id,))
            ...
            cur.execute("""
                ...
                SET caller_number = CASE
                    ...
                    END
                WHERE id = %s
            """, (phone, name or "", call_id))

        conn.commit()

    _upsert_profile(uid, phone, name=name)
    _refresh_customer_analysis(uid, phone, reason="call_summary")

    logger.info("[CustomerLink] linked call_id=%s phone=%s", call_id, phone[-4:])
    return True
