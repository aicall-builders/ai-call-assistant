...
def _event_payload_from_call(call_row, request_body=None):
    ...
    Priority:
    1. Explicit request body event fields
    2. extracted_info.calendar_candidate generated by NLP
    3. legacy extracted_info date/time fields
    4. demo fallback tomorrow 19:00
    """
    ...
    candidate = info.get("calendar_candidate") or {}
    ...
    date_value = (
        ...
        or candidate.get("reservation_date")
    ...
    time_value = (
        ...
        or candidate.get("reservation_time")
    ...
    try:
        duration_min = int(
            manual.get("duration_minutes")
            or candidate.get("duration_minutes")
            or info.get("duration_minutes")
            or CALENDAR_DEFAULT_DURATION_MINUTES
        )
    except Exception:
    ...
    party_size = (
        manual.get("party_size")
        or manual.get("people")
        or candidate.get("party_size")
        or candidate.get("people")
        or info.get("party_size")
        or info.get("people")
        or info.get("persons")
    )

    title = manual.get("title") or candidate.get("title")
    if not title:
        title_bits = ["예약"]
        if customer:
    ...
    desc_lines = [
        ...
        or "AI 통화비서가 통화 카드에서 생성한 예약 일정입니다."
    ]
    ...
    special_notes = (
        manual.get("special_notes")
        or candidate.get("special_notes")
        or info.get("special_notes")
    )

    if special_notes:
        desc_lines.append(f"특이사항: {special_notes}")
