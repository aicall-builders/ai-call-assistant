...
def _fetch_profile(provider, access_token, id_token=None):
    if provider == "google":
        # Android에서 idToken을 provider_access_token으로 보내는 경우 처리
        # idToken은 점(.)이 2개 있는 JWT 형식
        effective_id_token = id_token or (access_token if access_token.count('.') == 2 else None)
        
        if effective_id_token:
            res = requests.get(
                "https://oauth2.googleapis.com/tokeninfo",
                params={"id_token": effective_id_token},
                timeout=10,
            )
            if res.status_code < 400:
                data = res.json()
        ...
        # fallback: access_token으로 userinfo 조회
        res = requests.get(
...
def _handle_provider_login(event, provider):
    ...
    try:
        ...
        if not access_token:
            ...
            id_token = token_data.get("id_token") if provider == "google" else None
